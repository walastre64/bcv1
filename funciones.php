...
function actualizar_tr_pend($codMoneda1,$codMoneda2,$codMoneda3){
    include "connect.php";
...
sELECT [c_codmoneda]
    ...
    ,[n_Porc_IGTF]
    ,[c_Modalidad_IGTF]
FROM [192.168.23.7].[VAD10_PRUEBAS].[dbo].[MA_MONEDAS] where c_codmoneda in ('$codMoneda1','$codMoneda2','$codMoneda3')
    ");
     $resultado = $sentencia->execute(); # Pasar en el mismo orden de los ?
            if ($resultado === true) {
...
function actualizar_tr_pend_farmacia($codMoneda1,$codMoneda2,$codMoneda3){
    include "connect_Farma.php";
...
SELECT [c_codmoneda]
    ...
    ,[n_Porc_IGTF]
    ,[c_Modalidad_IGTF]
FROM [DCBAHIA\BAHIA].[VAD10_PRUEBAS_F].[dbo].[MA_MONEDAS] where c_codmoneda in ('$codMoneda1','$codMoneda2','$codMoneda3')
    ");
     $resultado = $sentencia->execute(); # Pasar en el mismo orden de los ?
            if ($resultado === true) {
...
function actualizar_historico_farmacia($valor,$codMoneda){
	...
	/*else{
		...
		insert into [DCBAHIA\BAHIA].[VAD10_PRUEBAS_F].[dbo].[MA_HISTORICO_MONEDAS]
		  ...
		  ,[TxtPlus2]
		  ,[c_CodUsuario]
		FROM [DCBAHIA\BAHIA].[VAD10_PRUEBAS].[dbo].[MA_HISTORICO_MONEDAS]
		where c_CodMoneda = '$codMoneda'
		order by d_FechaCambioActual desc  ");
