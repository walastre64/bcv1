...
sELECT [c_codmoneda]
    ...
    ,[n_Porc_IGTF]
    ,[c_Modalidad_IGTF]
FROM [VAD10_PRUEBAS].[dbo].[MA_MONEDAS] where c_codmoneda in ('$codMoneda1','$codMoneda2')
    ");
     $resultado = $sentencia->execute(); # Pasar en el mismo orden de los ?
            if ($resultado === true) {
...
SELECT TOP (1) 
      ...
      ,[TxtPlus2]
      ,[c_CodUsuario]
  FROM [VAD10_PRUEBAS].[dbo].[MA_HISTORICO_MONEDAS]
  where c_CodMoneda = '$codMoneda'
...
insert into [DCBAHIA\BAHIA].[VAD10_PRUEBAS].[dbo].[TR_PEND_MONEDAS]
SELECT [c_codmoneda]
    ,[c_descripcion]
    ,[n_factor]
    ,[b_preferencia]
    ...
    ,[n_Porc_IGTF]
    ,[c_Modalidad_IGTF]
FROM [DCBAHIA\BAHIA].[VAD10_PRUEBAS].[dbo].[MA_MONEDAS] where c_codmoneda in ('$codMoneda1','$codMoneda2')
    ");
     $resultado = $sentencia->execute(); # Pasar en el mismo orden de los ?
            if ($resultado === true) {
...
function actualizar_historico_farmacia($valor,$codMoneda){
    ...
    $sentencia = $base_de_datos->prepare("
    insert into [DCBAHIA\BAHIA].[VAD10_PRUEBAS].[dbo].[MA_HISTORICO_MONEDAS]
SELECT TOP (1) 
		[d_FechaCambioActual] 
		,getdate()
      ,[c_CodMoneda]
